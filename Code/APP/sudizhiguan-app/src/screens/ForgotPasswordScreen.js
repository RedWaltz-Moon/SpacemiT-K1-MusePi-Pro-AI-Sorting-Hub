...
export default function ForgotPasswordScreen({ navigation }) {
  ...
  async function handleReset() {
    ...
    if (captcha.trim() !== quiz.answer) {
      setError('验证码错误'); refreshCaptcha(); return;
    }
    setLoading(true); setError('');
    try {
      const res = await resetPassword(phone, password, confirm);
    ...
    <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.flex}>
        <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={s.input} placeholder="请输入注册手机号" keyboardType="number-pad" value={phone} onChangeText={setPhone} />
          ...
          <TextInput style={s.input} placeholder="再次输入新密码" secureTextEntry value={confirm} onChangeText={setConfirm} />
          ...
          <TouchableOpacity style={s.btn} onPress={handleReset} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>重置密码</Text>}
          </TouchableOpacity>

          <TouchableOpacity style={s.link} onPress={() => navigation.goBack()}>
            <Text style={s.linkText}>想起密码了？去登录</Text>
          </TouchableOpacity>
        </ScrollView>
